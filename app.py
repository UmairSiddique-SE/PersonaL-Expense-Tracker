...
def download_report():
    ...
    try:
        ...
        app.logger.exception("Report query error"); items = []
    filtered, totals, category_totals = summarize_items(items, view_type, selected_date, from_date, to_date, active_tab, selected_category)
    ...
    for item in chronological:
        t, amount = normalize_transaction_type(item.get("type")), money(item.get("amount"))
    ...
    buffer = io.BytesIO()
    try:
        doc = SimpleDocTemplate(buffer, pagesize=A4, topMargin=12*mm, bottomMargin=14*mm, leftMargin=10*mm, rightMargin=10*mm, title="Expense Tracker Financial Report", author=normalize_text(session.get("first_name","User"),80))
        styles = getSampleStyleSheet()
        title = ParagraphStyle("rt", parent=styles["Title"], fontSize=19, leading=23, textColor=colors.HexColor("#0f172a"), spaceAfter=5)
        section = ParagraphStyle("rs", parent=styles["Heading3"], fontSize=11.5, leading=14, fontName="Helvetica-Bold", textColor=colors.HexColor("#0f172a"), spaceBefore=8, spaceAfter=6)
        meta = ParagraphStyle("rm", parent=styles["Normal"], fontSize=8.2, leading=11, textColor=colors.HexColor("#475569"), spaceAfter=9)
        cell = ParagraphStyle("rc", parent=styles["Normal"], fontSize=6.8, leading=8.5, textColor=colors.HexColor("#0f172a"))
        ...
        card_label = ParagraphStyle("cl", parent=styles["Normal"], fontSize=7.5, leading=9, fontName="Helvetica-Bold", textColor=colors.white, alignment=1)
        card_value = ParagraphStyle("cv", parent=styles["Normal"], fontSize=11, leading=13, fontName="Helvetica-Bold", textColor=colors.white, alignment=1)
        ...
        top.setStyle(TableStyle([("BACKGROUND",(0,0),(0,-1),colors.HexColor("#059669")),("BACKGROUND",(1,0),(1,-1),colors.HexColor("#e11d48")),("BACKGROUND",(2,0),(2,-1),colors.HexColor("#2563eb")),("INNERGRID",(0,0),(-1,-1),.4,colors.white),("BOX",(0,0),(-1,-1),.6,colors.HexColor("#cbd5e1")),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7)]))
        elements += [top, Spacer(1,9), Paragraph("Category-wise Summary",section)]
        cats=[]
        ...
        for i in range(0,len(cats),3):
            ...
            for category,ci,ce,total in cats[i:i+3]:
                ...
                bg="#059669" if kind=="INCOME" else ("#e11d48" if kind=="EXPENSE" else "#6366f1")
        ...
        # PDF order requested by user: Category, Description, Date/Time, then amounts.
        ...
        elements += [table, Spacer(1,10), Paragraph("Final Summary",section), top]
