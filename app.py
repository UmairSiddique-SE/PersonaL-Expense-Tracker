...
def download_report():
    ...
    for item in chronological:
        t = normalize_transaction_type(item.get("type")); amount = money(item.get("amount"))
        inc = amount if t == "income" else Decimal("0.00"); exp = amount if t == "expense" else Decimal("0.00")
        if t == "income": running_balance += amount
        elif t == "expense": running_balance -= amount
        ledger.append((item, inc, exp, running_balance))
    buffer = io.BytesIO()
    try:
        doc = SimpleDocTemplate(buffer, pagesize=A4, topMargin=12*mm, bottomMargin=14*mm, leftMargin=12*mm, rightMargin=12*mm, title="Expense Tracker Financial Report", author=normalize_text(session.get("first_name", "User"), 80))
        ...
        value = ParagraphStyle("rv", parent=styles["Normal"], fontSize=10.5, leading=13, fontName="Helvetica-Bold", alignment=2)
        ...
        summary.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#f1f5f9")),("GRID",(0,0),(-1,-1),.5,colors.HexColor("#cbd5e1")),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("TOPPADDING",(0,0),(-1,0),6),("BOTTOMPADDING",(0,0),(-1,0),6),("TOPPADDING",(0,1),(-1,1),8),("BOTTOMPADDING",(0,1),(-1,1),8)]))
        elements += [summary, Spacer(1,4), Paragraph("Category-wise Summary", section)]
        cats = []
        for category, value_data in category_totals.items():
            ci = money(value_data.get("income")); ce = money(value_data.get("expense")); total = ci + ce
        ...
        for category,ci,ce,total in cats:
            cat_data.append([Paragraph(escape(category),cell),Paragraph(f"{ci:,.2f}" if ci else "—",inc_style),Paragraph(f"{ce:,.2f}" if ce else "—",exp_style),Paragraph(f"{total:,.2f}",bal_style)])
        ...
        cat_table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#2563eb")),("GRID",(0,0),(-1,-1),.35,colors.HexColor("#cbd5e1")),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f8fafc")]),("ALIGN",(1,1),(-1,-1),"RIGHT"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LEFTPADDING",(0,0),(-1,-1),5),("RIGHTPADDING",(0,0),(-1,-1),5),("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5)]))
        ...
        for index,(item,ia,ea,balance) in enumerate(ledger,1):
            ...
            data.append([Paragraph(str(index),cell),Paragraph(escape(dt),cell),Paragraph(escape(desc),cell),Paragraph(escape(category),cell),Paragraph(f"{ia:,.2f}" if ia else "—",inc_style),Paragraph(f"{ea:,.2f}" if ea else "—",exp_style),Paragraph(f"{balance:,.2f}",bal_style)])
        ...
        ledger_table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#0f172a")),("GRID",(0,0),(-1,-1),.35,colors.HexColor("#cbd5e1")),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#f8fafc")]),("VALIGN",(0,0),(-1,-1),"TOP"),("ALIGN",(4,1),(-1,-1),"RIGHT"),("LEFTPADDING",(0,0),(-1,-1),3),("RIGHTPADDING",(0,0),(-1,-1),3),("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4)]))
        elements += [ledger_table, Spacer(1,9), Paragraph("Final Summary", section)]
        final=Table([[Paragraph("Total Income",final_head),Paragraph("Total Expense",final_head),Paragraph("Final Balance",final_head)],[Paragraph(f"Rs {total_income:,.2f}",ParagraphStyle("f1",parent=final_val,textColor=colors.HexColor("#059669"))),Paragraph(f"Rs {total_expense:,.2f}",ParagraphStyle("f2",parent=final_val,textColor=colors.HexColor("#e11d48"))),Paragraph(f"Rs {running_balance:,.2f}",ParagraphStyle("f3",parent=final_val,textColor=colors.HexColor("#2563eb")))]],colWidths=[58*mm,58*mm,58*mm])
        final.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#e2e8f0")),("BACKGROUND",(0,1),(0,1),colors.HexColor("#ecfdf5")),("BACKGROUND",(1,1),(1,1),colors.HexColor("#fff1f2")),("BACKGROUND",(2,1),(2,1),colors.HexColor("#eff6ff")),("BOX",(0,0),(-1,-1),.6,colors.HexColor("#cbd5e1")),("INNERGRID",(0,0),(-1,-1),.4,colors.HexColor("#cbd5e1")),("TOPPADDING",(0,0),(-1,0),7),("BOTTOMPADDING",(0,0),(-1,0),7),("TOPPADDING",(0,1),(-1,1),8),("BOTTOMPADDING",(0,1),(-1,1),8),("VALIGN",(0,0),(-1,-1),"MIDDLE")]))
        elements.append(final)
        def footer(canvas,document):
            canvas.saveState(); canvas.setFont("Helvetica",7); canvas.setFillColor(colors.HexColor("#64748b")); canvas.drawRightString(A4[0]-12*mm,7*mm,f"Page {document.page}"); canvas.restoreState()
